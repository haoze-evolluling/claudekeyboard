...
class TouchpadFragment : Fragment() {
    ...
    private fun setupViews(root: View) {
        val backToHome = root.findViewById<TextView>(R.id.tv_back_to_home)
        ...
        toKeyboard.text = getString(R.string.tab_keyboard)
    ...
    private fun releaseDrag() {
        ...
        getMouseSender()?.let { sender ->
            Thread {
                sender.sendMouseMove(0, 0)  // sends a zeroed report to release all buttons
            }.start()
        }
    }
    ...
    private fun getMouseSender(): MouseSender? {
        return bluetoothViewModel.mouseSender.value
    }
